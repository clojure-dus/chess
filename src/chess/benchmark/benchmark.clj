...
(def allboards-vector (flatten (repeat (map #(read-fen % {:move-logic :vector}) boards-fen))))

(def allboards-bitboard (flatten (repeat (map #(read-fen % {:move-logic :bitboard}) boards-fen))))

(defn measure [x f allboards]
  (let [ boards (take x allboards) ]
    (with-progress-reporting (quick-bench (doseq [board boards] (f board)) :verbose))))


(comment

(measure 400 generate-moves  allboards-bitboard )

(measure 400 generate-moves  allboards-vector )

(measure 400 rate allboards-bitboard)

(measure 400 rate allboards-vector)

(measure 3 select-move allboards-bitboard)

(measure 3 select-move allboards-vector))
