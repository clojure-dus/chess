(ns chess.benchmark
  ...
  (:use [chess.move-selection :only (select-move)]))
...
(def bitboard-move-gen (bitboard/move-generator))
(def default-move-gen (default/move-generator))


(def allboards (flatten (repeat (map (partial read-fen default-move-gen) boards-fen))))

(def allboards-bb (flatten (repeat (map  (partial read-fen bitboard-move-gen) boards-fen))))
...
(defn measure-moves []
  (measure 400 (partial generate-moves default-move-gen) allboards))


(defn measure-bitmap-moves []
  (measure 400 (partial generate-moves bitboard-move-gen) allboards-bb))
